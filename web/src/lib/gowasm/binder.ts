import { type CallImportHandler, type GoWrapper } from '~/lib/go/wrapper/wrapper'
...
interface GoExportMetadata {
  packageName: string
  symbols: Array<[string, CallImportHandler]>
}

const defineExportMetadata = (target: any, meta: GoExportMetadata) => {
  Object.defineProperty(target, goExportMetadataKey, {
    configurable: false,
    enumerable: false,
    writable: false,
    value: meta,
...
export const Package =
  (pkgName: string): ClassDecorator =>
  (constructor: Function) => {
    const meta = getGoExportMetadata(constructor.prototype)
    if (meta) {
...
export const WasmExport =
  ...
  (target, propertyKey, descriptor) => {
    ...
    if (meta) {
      meta.symbols.push([funcName, func])
      return
    }
...
export const registerExportObject = (go: GoWrapper, srcObj: PackageBinding) => {
  ...
  if (!meta) {
    throw new Error(`Go export metadata is missing.`)
  }

  const { packageName, symbols } = meta
  for (const [funcName, func] of symbols) {
    go.exportFunction(`${packageName}.${funcName}`, (sp, reader, mem) => func.call(srcObj, sp, reader, mem))
  }
}
