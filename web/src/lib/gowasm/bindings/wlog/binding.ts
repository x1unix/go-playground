...
import { type MemoryView, type SliceHeader, SliceHeaderType, type StackReader, stringDecoder, Uint8 } from '~/lib/go'
import { ConsoleLogger, type Logger } from '~/lib/gowasm/bindings/wlog/logger'
...
export class LoggerBinding extends PackageBinding {
  ...
  logWrite(sp: number, stack: StackReader, mem: MemoryView) {
    ...
    const msg = stringDecoder.decode(mem.get(msgSlice.data, msgSlice.len, false))
