...
import { type Inode, MAX_FILE_NAME_LEN, TInode } from './types'
...
const validateFileName = (name: string) => {
  checkFileNameLimit(name.length)
}
...
export class BrowserFSBinding extends PackageBinding {
  ...
  readDir(sp: number, reader: StackReader, mem: MemoryView) {
    ...
    this.helper.doAsync(cbId, async () => {
      ...
      mem.write<Inode[]>(outSlice.data, new ArrayTypeSpec(TInode, items.length), inodes)
    })
  }
