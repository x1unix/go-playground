...
registerRoute(
  ...
  new StaleWhileRevalidate({
    cacheName: 'wasm',
    plugins: [new ExpirationPlugin({ maxAgeSeconds: 5 * DAY_IN_SECONDS })],
  }),
)

const apiEndpointsForCaching = new Set(['/api/backends/info', '/api/version'])

// Cache available Go versions from Go playground
registerRoute(
  ({ url }) => url.origin === self.location.origin && apiEndpointsForCaching.has(url.pathname),
  new NetworkFirst({
    cacheName: 'api',
